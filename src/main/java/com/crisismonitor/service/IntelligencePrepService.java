 ...
 * for all monitored countries. Stores enriched data in Redis so country briefings
 ...
 * Runtime: ~5-8 minutes for all 47 countries.
 * Output: Redis keys "intel:prepared:{iso3}" with 24h TTL.
 */
@Slf4j
@Service
...
public class IntelligencePrepService {
    ...
    private List<ArticleIntel> fetchNewsWithSnippets(String countryName, String iso3) {
        ...
        try {
            // Search with crisis-focused query for better relevance
            ...
            List<NewsAggregatorService.RssItem> rawItems = new ArrayList<>();

            for (NewsAggregatorService.RssItem item : feed.getChannel().getItems()) {
                if (rawItems.size() >= ARTICLES_PER_COUNTRY) break;
            ...
            // Phase 1: Build articles from RSS data (title + RSS description)
            for (NewsAggregatorService.RssItem item : rawItems) {
                ArticleIntel article = new ArticleIntel();
                article.title = cleanTitle(item.getTitle());
                article.url = item.getLink();
                article.source = extractSource(item.getTitle());
                article.date = item.getPubDate();

                // Bing RSS description often contains a useful snippet
            ...
            // Phase 2: For top 3 articles that have no snippet, fetch actual page content
            ...
            // Wait for page fetches with timeout
            ...
            log.warn("Bing News fetch failed for {}: {}", countryName, e.getMessage());
        }
    ...
    /**
     * Prepared intelligence for a single country, stored in Redis.
     */
